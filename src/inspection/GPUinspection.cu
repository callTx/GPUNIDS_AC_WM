...
void GPUInspection::exec(std::shared_ptr<Packet> pkt)
{
    ...
    if( gettimeofday(&(pkt.get()->virtualTime), nullptr) != 0)
    {
    ...
    if(proc)
    {
        ...
        checkCudaErrors(cudaStreamSynchronize(stream_[id]));
        if( gettimeofday(&end, nullptr) != 0)
        {
            std::cerr << "Fail to get current time" << std::endl;
            exit(-1);
...
int GPUInspection::buildMatchingMachine(std::string rules, int bufferSize, int nThreads)
{

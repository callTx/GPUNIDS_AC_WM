...
do
echo "CPU $througput" >> $DIR/resources.csv
echo "Data,CPU(%),Mem(%),Tempo" >> $DIR/resources.csv
echo "Starting GPUNIDS"
echo "Running in: CPU"
echo "Througput : $througput"
echo "Througput : $througput" >> $DIR/output.txt
../build/./GPUNIDS -i p2p1 -r rules.csv -f "tcp port 5201 and ip src 10.0.0.1" -p S -l 10 -t 4 >> $DIR/output.txt & pid=$!
...
do
    ...
    echo `date +%Y-%m-%d:%H:%M:%S`","$cpu","$ram","$elapsed_time >> $DIR/resources.csv
    sleep 1s
done
...
echo "CPU 1gb" >> $DIR/resources.csv
echo "Data,CPU(%),Mem(%),Tempo" >> $DIR/resources.csv
echo "Starting GPUNIDS"
echo "Running in: CPU"
echo "Througput : 1gb"
